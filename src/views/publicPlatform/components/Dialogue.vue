	...
	<el-row>
		<div id="main">
			<div id="image">
				...
				<img v-else-if="identified == false" class="self_adaption" :src="image" alt="图片显示错误" />
				<Mathdown v-else :content="confirmed_content" class="content"></Mathdown>
				<el-button type="primary" id="edit" @click="Open_Edit()">编辑
				</el-button>
				<div id="shadow">
					<div class="dialog">
						<el-input style="position: absolute;left:15px;top:20px;" type="text" size="large"
							placeholder="请输入url" v-model="url_input" clearable>
						</el-input>
					</div>
					<el-button type="primary" class="but" style="left:41.25%;" @click="url_update()">
						url上传
					</el-button>
					<!-- <span style="position:absolute; color: #FFFFFF; font-size: 20px; top:25px; left: 450px;">或</span> -->
					<el-button type="primary" class="but" style="left:57%;" @click="Open_Pic_Search()">
						本地上传
						<i class="el-icon-upload el-icon--right"></i>
					</el-button>
					<el-button type="primary" class="but" style="right:20px;" :disabled="identified"
						@click="identify()">
						识别
					</el-button>
				</div>
			...
			<el-dialog :visible.sync="EditShow" title="LUNA输入助手" width="80%" :modal-append-to-body="false"
				...
				<el-button type="primary" style="position:relative; left:500px;" @click="confirm()">
					确定
				</el-button>
			</el-dialog>
			...
			<div id="option">
				<div v-for="(img,index) in optional_image" :key="img" class="optional_img"
					:style="{'left' : index*25+2.5+'%'}" @click="ChooseDefaultImage(img)">
