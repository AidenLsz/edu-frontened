<template>
  ...
  >
    <div id="Filter" class="Top_Nav"></div>
    <LunaProgress
    ...
    <div class="Jump_Bar">
      <el-row class="Jump_Item" @click.native="Jump_To('Filter')">
    ...
    >
      <ComplexInput
        @New_Content="Update_Complex_Input"
        :Get_Out_Content="Search_Content"
      ></ComplexInput>
    </el-dialog>
    <!-- 图片检索的对话框 -->
    <el-dialog
    ...
    >
      <el-row
        type="flex"
        justify="start"
        style="margin-left: 2.4vw; margin-bottom: 20px"
      >
      ...
      <el-col>
        ...
        >
          <el-breadcrumb separator-class="el-icon-arrow-right">
            <el-breadcrumb-item
              :to="{ path: $store.getters.systemType == 2 ? '/itas' : '/' }"
            >
              首页
            </el-breadcrumb-item>
            <el-breadcrumb-item>
        ...
        >
          <span style="font-size: 4rem">搜索分析</span>
        </el-row>
        <div class="Background_Round"></div>
        <!-- 功能区 -->
        <el-row class="Padding_Width">
          <el-col :span="16" style="min-height: 320px">
            ...
            <el-row type="flex" justify="start" class="Filter_Line">
              ...
              <div class="Filter_Item_Shadow">
                <span
                  v-for="(
                    Subject_Item, Subject_Item_Index
                  ) in All_Options.Subject"
                  :key="'Filter_Subject_' + Subject_Item_Index"
                  :class="Focus_Filter('Subject', Subject_Item)"
                  :style="
                    Filter_Item('Subject', Subject_Item_Index, Subject_Item)
                  "
            ...
            <el-row type="flex" justify="start" class="Filter_Line">
              ...
              <div class="Filter_Item_Shadow">
                <span
                  v-for="(
                    Database_Item, Database_Item_Index
                  ) in All_Options.Database"
          ...
          >
            ...
            <div v-show="Img_All != ''" class="ImgSearchArea" style="">
              <el-image
            ...
            <el-row type="flex" justify="center" style="margin-top: 20px">
              ...
              <el-button
                type="primary"
                style="border-radius: 10px; background: #539dd9"
                @click="Img_Reset()"
    ...
    <el-row>
      <el-col>
        ...
        >
          ...
          <el-row>
            <div :style="Get_Card_Margin(Paper_Info_Index)">
              <el-row
                style="
                  width: 100%;
                  height: 64px;
              ...
              <el-row class="Question_Card" style="background: white">
                <SearchPaperItem
                  :Add="Paper_List.indexOf(Paper_Info.id) == -1"
    ...
    >
      ...
      >
        ...
        <div class="close_icon" @click="Delete_Paper_By_Icon(Paper.ID)">
          <i
            class="el-icon-close"
            style="color: #da1b1b; font-size: 20px; line-height: 26px"
          ></i>
        </div>
      </div>
      <div slot="reference">
        <i
          ...
          style="font-size: 40px; line-height: 70px; color: #539dd9"
        ></i>
        <div v-if="Paper_List.length > 0" class="num">
          {{ Paper_List.length }}
        </div>
      </div>
    </el-popover>
    <el-button class="upload-button" type="primary" @click="ToPaperAnalyse"
      >录入试卷</el-button
    >
    <el-button class="analyse-button" type="primary" @click="show_analyse_option = true">智能分析</el-button>
    <el-dialog :visible.sync="show_analyse_option" width="70%">
      <el-row>
        <el-col :span="12">
          <el-row>
            <el-button
...
import SearchPaperItem from "@/views/resourceAnalyse/components/SearchPaperItem";
...
export default {
  ...
  methods: {
    ...
    Filter_Item(Part, Index, Item) {
      let WIDTH =
        ["Database", "Semantic"].indexOf(Part) != -1 ? "105px" : "70px";
      let BORDER_LEFT = "";
      let BORDER_RIGHT = "";
      if (Part == "Semantic") {
        ...
        if (this.Chosen_Options[Part].indexOf(Item) == -1) {
          if (Index > 0 && Index < this.All_Options[Part].length - 1) {
            if (
              this.Chosen_Options[Part].indexOf(
                this.All_Options[Part][Index + 1]
              ) == -1
            ) {
            ...
            if (
              this.Chosen_Options[Part].indexOf(this.All_Options[Part][1]) == -1
            ) {
              BORDER_RIGHT = "1px solid #ccc";
            } else if (
              this.Chosen_Options[Part].indexOf(this.All_Options[Part][1]) != -1
            ...
            if (
              this.Chosen_Options[Part].indexOf(
                this.All_Options[Part][this.All_Options[Part].length - 2]
              ) == -1
            ) {
              BORDER_LEFT = "none";
            } else if (
        ...
        if (this.Chosen_Options.Database.indexOf(Item) == -1) {
          if (Index > 0 && Index < this.All_Options.Database.length - 1) {
            if (
              this.Chosen_Options.Database.indexOf(
                this.All_Options.Database[Index + 1].nick
              ) == -1
            ) {
              BORDER_RIGHT = "1px solid #ccc";
            } else if (
            ...
            if (
              this.Chosen_Options.Database.indexOf(
                this.All_Options.Database[1].nick
              ) == -1
            ) {
              BORDER_RIGHT = "1px solid #ccc";
            } else if (
            ...
            if (
              this.Chosen_Options.Database.indexOf(
                this.All_Options.Database[this.All_Options.Database.length - 2]
                  .nick
              ) == -1
            ) {
              BORDER_LEFT = "none";
          ...
          if (this.All_Options.Database.length > 1) {
            if (Index > 0 && Index < this.All_Options.Database.length - 1) {
              ...
              if (
                this.Chosen_Options.Database.indexOf(
                  this.All_Options.Database[
                    this.All_Options.Database.length - 2
                  ].nick
                ) == -1
              ) {
                BORDER_LEFT = "1px solid #409EFF";
      ...
      return {
        ...
        "border-top-right-radius":
          Index == this.All_Options[Part].length - 1 ? "10px" : "0px",
    ...
    Filter_Change(Part, Item) {
      if (["Period", "Subject", "Type"].indexOf(Part) != -1) {
        if (this.Chosen_Options[Part].indexOf(Item) == -1) {
          ...
          this.Chosen_Options[Part].splice(
            this.Chosen_Options[Part].indexOf(Item),
            1
          );
        }
      } else if (Part == "Difficulty" || Part == "Semantic") {
    ...
    InitDatabaseList() {
      ...
      commonAjax(this.backendIP + "/api/get_user_ig_name", {
        ...
        for (var i = 0; i < data.length; i++) {
          this.All_Options.Database.push({
            name: data[i],
            nick: "个人题库" + (data.length > 1 ? i + 1 + "" : ""),
          });
        }
      });
    },
    ...
    Search_Do() {
      ...
      for (let i = 0; i < this.Chosen_Options.Database.length; i++) {
        for (let j = 0; j < this.All_Options.Database.length; j++) {
          if (
            this.Chosen_Options.Database[i] == this.All_Options.Database[j].nick
          ) {
    ...
    Page_Index_Change_Check() {
      ...
      for (let i = 0; i < Keys.length; i++) {
        let key = Keys[i];
        if (
          JSON.stringify(this.History_Chosen_Options[key]) !=
          JSON.stringify(this.Chosen_Options[key])
        ) {
          Flag = true;
          this.History_Chosen_Options[key] = JSON.parse(
            JSON.stringify(this.Chosen_Options[key])
          );
        }
      }
      if (
        JSON.stringify(this.History_Chosen_Options.Img) !=
        JSON.stringify(this.Img_Cut)
      ) {
        Flag = true;
        this.History_Chosen_Options.Img = JSON.parse(
          JSON.stringify(this.Img_Cut)
        );
      }
      if (
        JSON.stringify(this.History_Chosen_Options.Search_Content) !=
        JSON.stringify(this.Search_Content)
      ) {
        Flag = true;
        this.History_Chosen_Options.Search_Content = JSON.parse(
          JSON.stringify(this.Search_Content)
        );
      }
      if (Flag) {
    ...
    Get_Card_Margin(Question_Index) {
      ...
      let Style = {
        margin:
          Question_Index == this.Paper_Info_List.length - 1
            ? Style_Row_0
            : Style_Row_1,
        width: "1344px",
      };
    ...
    Paper_Analyse(ID) {
      ...
      if (
        this.Paper_Cache_ID == ID &&
        sessionStorage.getItem("PaperDetailShow")
      ) {
    ...
    // 向试卷篮添加试卷
    Add_Paper(ID, title) {
      this.Paper_List.push({ ID: ID, title: title });
      // console.log("Paper_List", this.Paper_List);
    },
    Delete_Paper(ID) {
      // console.log(ID);
      function check(x) {
        // console.log("paper_list",x.ID);
        return x.ID == ID;
      }
      let k = this.Paper_List.findIndex(check);
      if (k == -1) return;
      this.Paper_List.splice(k, 1);
      // console.log("Paper_List", this.Paper_List);
    },
    Delete_Paper_By_Icon(ID) {
      this.Delete_Paper(ID);
    },
    ToPaperAnalyse() {
      this.$router.push({
    ...
    PAS(index) {
      console.log(index);
      this.show_analyse_option = false;
    }
  },
};
</script>
