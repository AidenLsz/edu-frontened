<template>
  ...
  <div class="">
    <el-dialog
    :visible.sync="CheckAuthorityVisible"  width="70%" style="margin-top: -13vh;">
      <el-row>
        <el-col :span="10" :offset="2">
          <span style="font-weight: bold; color: #47A2FF; font-size: 24px">
            身份验证
          </span>
          <el-form :model="ruleForm" status-icon :rules="rules" ref="checkAuthorityForm" label-position="left" label-width="90px" class="demo-ruleForm"
    ...
    <el-dialog
      ...
      <el-row>
        <el-col :span="10" :offset="2">
          ...
          <el-form :model="passRuleForm" status-icon :rules="passRules" ref="resetPassForm" label-position="left" label-width="90px" class="demo-ruleForm"
  ...
  methods:{
    ...
    resetPass(){
      this.$refs['resetPassForm'].validate((valid) => {
        ...
        let fd={
          ...
          password:this.passRuleForm.password,
        }
        commonAjax(this.backendIP + "/api/reset_password",fd).then((res)=>{
          console.log(res);
    ...
    CheckAuthority(){
      this.$refs['checkAuthorityForm'].validate((valid) => {
        ...
        commonAjax(this.backendIP + "/api/forget_pass_check",fd).then((res)=>{
